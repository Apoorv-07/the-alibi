...
class Presentation:
    ...
    def date_words(self, value) -> str:
        """A date in a policy or config file, phrased — deliberately *not* with `when_phrase`, which says
        "Due tomorrow": right for a task, wrong for a freeze date nobody has to act on. An absent or
        unreadable value is named as absent rather than reformatted into a date that looks authoritative.
        """
        if not value:
            return "not set in this policy"
        try:
